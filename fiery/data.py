...
class ArgoverseFPD(torch.utils.data.Dataset):
	def __init__(self, cfg, datapath='data/argoverse-train', train=False):
		...
		self.image_size_hw = (self.image_height, self.image_width)

		self.image_resize_scale = self.cfg.IMAGE.RESIZE_SCALE
		self.image_final_size_hw = self.cfg.IMAGE.FINAL_DIM
		self.camera_names = self.dataset.CAMERA_LIST
	...
	def __form_frames_sequences(self) -> np.array[Dict[str, List[int]]]:
		sequences = []		
		for log in self.scene_logs:
			...
			for idx in range(0, n_images - self.frames_in_future):
				sequence = {'log': log, 'frames_idxs': [i for i in range(idx, idx + self.sequence_length)]}
				sequences.append(sequence)

		return np.asarray(sequences)
	
	def __get_resizing_and_cropping_parameters(self) -> \
		Dict[Tuple[int, int], float, float, Tuple[int, int, int, int], Tuple[int, int]]:

		original_height, original_width = self.image_size_hw
		final_height, final_width = self.image_final_size_hw

		resize_scale = min(original_width//final_width, original_height//final_height)
		resize_scale = 1/resize_scale
		resized_dims = (original_height * resize_scale, original_width * resize_scale)

		crop_h = int(resized_dims[0] - final_height)
		crop_w = int(max(0, (resized_dims[1] - final_width) / 2))

		# How many pixels to be cropped from resized image (left, top, bottom, right)
		crop_pixels = (crop_w, crop_h, 0, crop_w)

		return {'orig_dims': self.image_size_hw,
				'scale_width': resize_scale,
				'scale_height': resize_scale,
				'crop_pixels_final_image': crop_pixels,
				'final_dims': self.image_final_size_hw
				}

	def __get_input_data(self, sequence: Dict[str, List[int]]) \
		-> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
		"""
		Parameters
		----------
			sequence: dict{'log': log of sequence in argoverse dataset, 'frames_idxs': frames idecies in log}
	...
	def __getitem__(self, index):
		...
		instance_map = {}
		# Loop over all the frames in the sequence.
		for sequence in self.frames_sequences:

			images, intrinsics, extrinsics = self.__get_input_data(sequence)
			segmentation, instance, z_position, instance_map, attribute_label = self.get_label(rec, instance_map)
			future_egomotion = self.get_future_egomotion(rec, index_t)
