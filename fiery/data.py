...
class FuturePredictionDataset(torch.utils.data.Dataset):
	def __init__(self, dataset, is_train, cfg):
		...
		self.is_argoverse = isinstance(dataset, ArgoverseTrackingLoader)

		if self.is_lyft:
			self.dataroot = self.dataset.data_path
		elif self.is_argoverse:
			self.dataroot = self.dataset.root_dir
		else:
...
def prepare_dataloaders(dataset_name, cfg, return_dataset=False):
	...
	if cfg.DATASET.NAME == 'nuscenes':
		...
		dataset = LyftDataset(data_path=dataroot,
						   ...
						   verbose=True)
	elif cfg.DATASET.NAME == 'argoverse':
		dataroot = os.path.join(cfg.DATASET.DATAROOT)
		dataset = ArgoverseTrackingLoader(dataroot)
